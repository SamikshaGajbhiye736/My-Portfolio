...
(function ($) {
    ...
    $("#loading").fadeOut(500); // hide preloader quickly
});
...
// ======= CUSTOMIZE THESE =======
const PHONE_NUMBER = '919000056780'; // international format without +, e.g. 919876543210
const AUTO_MESSAGE = 'Hi! I saw your website and would like to know more about your services.';
// ================================


function openWhatsApp(phone, message) {
const encoded = encodeURIComponent(message);
// Using wa.me link that works on mobile & desktop (opens WhatsApp Web on desktop)
...
e.preventDefault();
openWhatsApp(PHONE_NUMBER, AUTO_MESSAGE);
});


// Optional: keyboard shortcut (shift+W) to open WhatsApp quickly
...
window.addEventListener('load', () => {
  // website full load झाल्यानंतर 10 सेकंदानंतर popup show होईल
  setTimeout(() => {
    const popup = document.getElementById('popup');
    popup.style.display = 'flex';
    popup.classList.add('show'); // optional animation
  }, 10000);

  // Close button
  document.querySelector('#popup .btn-close-popup').addEventListener('click', () => {
    const popup = document.getElementById('popup');
...
document.addEventListener("DOMContentLoaded", () => {
  const openBtn = document.querySelector(".rr-menu-bar");
  const offcanvas = document.querySelector(".rroffcanvas-area");
